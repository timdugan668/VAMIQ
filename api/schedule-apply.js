export default async function handler(req, res) {
  ...
  try {
    if (action === 'reset') {
      ...
      if (!delRes.ok) {
        let detail = '';
        try { detail = await delRes.text(); } catch (e2) {}
        return res.status(500).json({ error: 'Could not reset day (status ' + delRes.status + '): ' + detail });
      }
    ...
    if (!upsertRes.ok) {
      let detailText = '';
      try { detailText = await upsertRes.text(); } catch (e2) {}
      return res.status(500).json({ error: 'Could not save (status ' + upsertRes.status + '): ' + detailText });
    }
